...
static void blink1_thread(void)
{
    int aux = 0;
...
static void blink2_thread(void)
{
    int aux = 0;
	while(bln_suspended && aux < bln_blinking_time)
	{
...
{
	if (!bln_enabled || !bln_suspended || bln_ongoing)
		return;

	bln_ongoing = true;
	bln_power_on();
	if(bln_blink_mode = 1)
		kthread_run(&blink1_thread, NULL,"bln_blink1_thread");
	else if(bln_blink_mode = 2)
		kthread_run(&blink2_thread, NULL,"bln_blink2_thread");
	else 
